  ...
  if (compact) {
    return (
      <div className="mt-2 flex flex-row gap-2 text-xs text-gray-500 items-center">
        <span title="1h volatility">1h: {fmt(volatility.volatility_1h)}</span>
        <span title="6h volatility">6h: {fmt(volatility.volatility_6h)}</span>
        <span title="24h volatility">24h: {fmt(volatility.volatility_24h)}</span>
  ...
  return (
    <div className="mt-4 p-2 border rounded">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <span>1h: {fmt(volatility.volatility_1h)}</span>
        <span>6h: {fmt(volatility.volatility_6h)}</span>
        <span>24h: {fmt(volatility.volatility_24h)}</span>
        <span>7d: {fmt(volatility.volatility_7d)}</span>
        <span>Normalized: {fmt(volatility.normalized_volatility)}</span>
